...
### Running the AWS SSM tests

Most of the SSM coverage runs against a stubbed AWS client and needs no credentials or setup.

`test/providers/ssm_test.rb` is the exception: it performs a live round trip against AWS SSM Parameter
Store, and skips itself unless `AWS_ACCESS_KEY_ID` is set. To run it, export AWS credentials, and to
exercise the assumed role portion of that test also export the AWS account id that holds the
`secret_config_test` role:

    export SECRET_CONFIG_ACCOUNT_ID=111111122222222

This env var is only read by that test. The library itself never looks at it.
